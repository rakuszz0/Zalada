...
export class Init1701411966381 implements MigrationInterface {
    name = 'Init1701411966381'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`username\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`phone_number\` int NOT NULL, \`address\` varchar(255) NOT NULL, \`userLevelId\` int NULL, \`cartsId\` int NULL, \`transactionsId\` int NULL, UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`users\` ADD CONSTRAINT \`FK_e5e7b3ae5f262fa9655ea036f61\` FOREIGN KEY (\`cartsId\`) REFERENCES \`carts\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`users\` ADD CONSTRAINT \`FK_050023cb44da55daa5f99609d2d\` FOREIGN KEY (\`transactionsId\`) REFERENCES \`transactions\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`users\` DROP FOREIGN KEY \`FK_050023cb44da55daa5f99609d2d\``);
        await queryRunner.query(`ALTER TABLE \`users\` DROP FOREIGN KEY \`FK_e5e7b3ae5f262fa9655ea036f61\``);
